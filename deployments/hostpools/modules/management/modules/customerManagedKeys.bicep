...
module storageAccountKeyVaults 'keyVault.bicep' = [for i in range(0, storageCount): {
  name: 'Storage_KeyVault_${string(padLeft(i + storageIndex, 2, '0'))}_${timeStamp}'
  params: {
    envShortName: envShortName
    keyVaultName: replace(storageKeyVaultNameConv, 'INDEX', string(padLeft(i + storageIndex, 2, '0')))
...
module keysStorageAccounts 'keyVaultKeys.bicep' = [for i in range(0, storageCount): {
  name: 'EncryptionKey_StorageAccount_${string(padLeft(i + storageIndex, 2, '0'))}_${timeStamp}'
  params: {
    exportable: false
