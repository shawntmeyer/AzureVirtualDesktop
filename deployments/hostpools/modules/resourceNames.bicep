...
var nameConv_HP_ResGroups = nameConvResTypeAtEnd ? 'avd-${hostPoolBaseName}-RESGROUPPURPOSE-${nameConvSuffix}' : 'RESOURCETYPE-avd-${hostPoolBaseName}-RESGROUPPURPOSE-${nameConvSuffix}'
var nameConv_HP_Resources = '${hostPoolPrefix}-${nameConvSuffix}'

// shared resources between host pools of same business unit
var nameConv_Shared_ResGroups = nameConvResTypeAtEnd ? ( !empty(busUnitId) ? 'avd-${busUnitId}-RESGROUPPURPOSE-${nameConvSuffix}' : 'avd-RESGROUPPURPOSE-${nameConvSuffix}' ) : ( !empty(busUnitId) ? 'RESOURCETYPE-avd-${busUnitId}-RESGROUPPURPOSE-${nameConvSuffix}' : 'RESOURCETYPE-avd-RESGROUPPURPOSE-${nameConvSuffix}' )
...
var globalFeedResourceGroupName = avdPrivateLink ? replace(replace((nameConvResTypeAtEnd ? 'avd-global-feed-${nameConvSuffix}' : 'RESOURCETYPE-avd-global-feed-${nameConvSuffix}'), 'LOCATION', '${locations[locationGlobalFeed].abbreviation}'), 'RESOURCETYPE', '${resourceAbbreviations.resourceGroups}') : ''
var globalFeedWorkspaceName = avdPrivateLink ? replace((nameConvResTypeAtEnd ? 'avd-global-feed-RESOURCETYPE' : 'RESOURCETYPE-avd-global-feed'), 'RESOURCETYPE', resourceAbbreviations.workspaces) : ''
...
var keyVaultNameConv = replace(replace('${hostPoolPrefix}-KEYVAULTPURPOSE-INDEX-${nameConvSuffix}', 'RESOURCETYPE', resourceAbbreviations.keyVaults), 'LOCATION', locations[locationVirtualMachines].abbreviation)
var keyVaultNameVMs = length(replace(replace(keyVaultNameConv, 'KEYVAULTPURPOSE-', 'vm-'), 'INDEX-', '')) > 24 ? replace(replace(replace(keyVaultNameConv, 'KEYVAULTPURPOSE-', 'vm-'), 'INDEX-', ''), '-', '') : replace(replace(keyVaultNameConv, 'KEYVAULTPURPOSE-', 'vm-'), 'INDEX-', '') 
var keyVaultNameConvStorage = length(replace(keyVaultNameConv, 'KEYVAULTPURPOSE-', 'st-')) > 27 ? replace(replace(keyVaultNameConv, 'KEYVAULTPURPOSE-', 'st-'), '-', '') : replace(keyVaultNameConv, 'KEYVAULTPURPOSE-', 'st-')
...
// Private Endpoints
var privateEndpointNameConvTemp = nameConvResTypeAtEnd ? 'RESOURCE-SUBRESOURCE-VNETID-RESOURCETYPE' : 'RESOURCETYPE-RESOURCE-SUBRESOURCE-VNETID'
var privateEndpointNameConv = replace(privateEndpointNameConvTemp, 'RESOURCETYPE', resourceAbbreviations.privateEndpoints)
var privateEndpointNICNameConvTemp = nameConvResTypeAtEnd ? '${privateEndpointNameConv}-RESOURCETYPE' : 'RESOURCETYPE-${privateEndpointNameConv}'
var privateEndpointNICNameConv = replace(privateEndpointNICNameConvTemp, 'RESOURCETYPE', resourceAbbreviations.networkInterfaces)
