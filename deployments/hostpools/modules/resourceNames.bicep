...
var diskEncryptionSetNameConv = replace(replace(replace(nameConv_HP_Resources, 'resourceType', resourceAbbreviations.diskEncryptionSets), 'location', locations[locationVirtualMachines].abbreviation), 'avd-', 'avd-desType-')
var diskEncryptionSetConfVms = length(replace(diskEncryptionSetNameConv, 'desType-', 'confvm-')) > 24 ? replace(replace(diskEncryptionSetNameConv, 'desType-', 'confvm-'), '-', '') : replace(diskEncryptionSetNameConv, 'desType-', 'confvm-')
var diskEncryptionSetCustKeysName = length(replace(diskEncryptionSetNameConv, 'desType-', 'vmckeys-')) > 24 ? replace(replace(diskEncryptionSetNameConv, 'desType-', 'vmckeys-'), '-', '') : replace(diskEncryptionSetNameConv, 'desType-', 'vmckeys-')
var diskEncryptionSetPlatAndCustKeysName = length(replace(diskEncryptionSetNameConv, 'desType-', 'vmpckeys-')) > 24 ? replace(replace(diskEncryptionSetNameConv, 'desType-', 'vmpckeys-'), '-', '') : replace(diskEncryptionSetNameConv, 'desType-', 'vmpckeys-')
var keyVaultNameConv = replace(replace(replace(nameConv_HP_Resources, 'resourceType', resourceAbbreviations.keyVaults), 'location', locations[locationVirtualMachines].abbreviation), 'avd-', 'avd-keyVaultPurpose-')
