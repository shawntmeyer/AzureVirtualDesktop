...
$stigZip = (Get-ChildItem -Path $PSScriptRoot -File -Filter '*.zip').FullName 
If (-not $stigZip) {
    ...
    If ($uriGPODownload) {
        $stigZip = Get-InternetFile -url $uriGPODownload -OutputDirectory $env:TEMP
    } Else {
        Write-Error "Cannot determine STIG source location from $uriSTIGs"
        Stop-Transcript
        Exit 2
    }
}
...
Expand-Archive -Path $stigZip -DestinationPath $outputDir
If ($uriGPODownload) {Remove-Item -Path $stigZip -Force}
...
$arrApplicableGPOs = Get-ChildItem -Path $outputDir | Where-Object {$_.Name -like "DoD*Windows $osVersion*" -or $_.Name -like 'DoD*Edge*' -or $_.Name -like 'DoD*Firewall*' -or $_.Name -like 'DoD*Internet Explorer*' -or $_.Name -like 'DoD*Defender Antivirus*'} 
...
#WN10-00-000175
Write-Output "WN10-00-000175/V-220732: Disabling the Secondary Logon Service."
$Service = 'SecLogon'
...
Write-Output "WN10-00-000145/V-220726: Checking to see if DEP is enabled."
    ...
    # WIN10-00-000210/220
    Write-Output 'WIN10-00-000210/220: Disabling Bluetooth Radios.'
