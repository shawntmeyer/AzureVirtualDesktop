function Get-SHRLatestImageVersion {
    ...
    if ($ImageReference.publisher) {
        ...
        if ($ImageReference.version -ne 'latest') {
            Write-OutputDetailed  "Image version is not set to latest. Returning version '$($ImageReference.version)'"
            $azImageVersion = $ImageReference.version
        }
        else {
            # Get the Images and select the latest version.           
            Write-OutputDetailed "Getting latest version of image publisher: $($ImageReference.publisher), offer: $($ImageReference.offer), sku: $($ImageReference.sku) in region: $($Location)"
            ...
            Write-OutputDetailed  "Latest version of image is $azImageVersion"
            ...
            if ($azImageVersion -match "\d+\.\d+\.(?<Year>\d{2})(?<Month>\d{2})(?<Day>\d{2})") {
                ...
                Write-OutputDetailed  "Image date is $azImageDate"
            } else {
    ...
    elseif ($ImageReference.Id) {
        ...
        Write-OutputDetailed "Image is from Shared Image Gallery: $($ImageReference.Id)"
        ...
        if ($ImageReference.Id -match $imageDefinitionResourceIdPattern) {
            Write-OutputDetailed 'Image reference is an Image Definition resource.'
            ...
            Write-OutputDetailed "Selected image version with resource Id {0}" -StringValues $latestImageVersion.Id
            $azImageVersion = $latestImageVersion.Name
            $azImageDate = $latestImageVersion.PublishingProfile.PublishedDate

            Write-OutputDetailed "Image version is {0} and date is {1}" -StringValues $azImageVersion, $azImageDate.ToString('o')

            # Switch back to original subscription
            if ($imageSubscriptionId -ne $currentSubscriptionId) {
                Write-OutputDetailed "Switching back to subscription {0}" -StringValues $currentSubscriptionId
                Set-AzContext -SubscriptionId $currentSubscriptionId
            }
        }
        elseif ($ImageReference.Id -match $imageVersionResourceIdPattern ) {
            Write-OutputDetailed 'Image reference is an Image Version resource.'
